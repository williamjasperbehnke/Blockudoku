...
namespace blockudoku
{
    ...
    void ui_renderer::render(const game_state& state)
    {
        ...
        draw_board(state);
        draw_tray(state);
        _feedback.update_careful_condition(state);
        _feedback.draw_messages(state, _text_generator, _selected_tray_generator, _text_sprites);
        _feedback.apply_shake(_gameplay_bg, _ui_bg, _text_sprites);
